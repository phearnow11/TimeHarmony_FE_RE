<template>
  <PopUp 
        ...
        />
 <nav
  v-if="(!auth.user_id && !['/login', '/signup', '/chat', '/upload', '/order', '/cart', '/forgot'].includes(route.path)) || (auth.user_id && !['/chat', '/admin' , '/order', '/orderconfirmation/:order_id'].includes(route.path))"
